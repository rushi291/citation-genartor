            ...
            if isinstance(source, dict):
                ...
                if isinstance(source_context, str):
                    ...
                    if response_doc.similarity(source_doc) > 0.7:  # Similarity threshold
                        found_citations.append({"id": source.get('id', ''), "link": source.get('link', '')})
            else:
                st.warning("Unexpected source format: expected a dictionary.")
        citations.append(found_citations)
    return citations
    ...
    if response_data:
        ...
        if citations:
            for citation in citations:
                if citation:  # Check if the citation list is not empty
                    for c in citation:
                        st.write(f"ID: {c['id']}, Link: {c['link']}")
                else:
